...
```./dsdgen -table <table-name> -scale <scale-factor> [-parallelism <parallelism> -child <child-number>]```

### Help

```
# java -jar tpcds-1.3-SNAPSHOT-jar-with-dependencies.jar -h
NAME
        dsdgen - data generator for TPC-DS

SYNOPSIS
        dsdgen [(--directory <directory> | -d <directory>)] [--do-not-terminate]
                [(-h | --help)] [--no-sexism] [--null <null>] [--overwrite]
                [--parallelism <parallelism>] [(--scale <scale> | -s <scale>)]
...
OPTIONS
        ...
        --no-sexism
            The reference C implementation picks only male names for the manager
            fields. This flag instructs our generator to pick managers from a
            distribution of male and female names. *The data set generated will
            differ from the one generated by the official C implementation.* But
            on the other hand it won't be sexist.
        ...
        --table <table>, -t <table>
            Build only the specified table. If this property is not specified,
            all tables will be generated
```
